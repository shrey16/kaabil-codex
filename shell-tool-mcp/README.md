...
Because `@kaabil/codex-shell-tool-mcp` intercepts `execve(2)` calls directly, it _always_ knows the full path to the program being executed. In turn, this makes it possible to provide stronger guarantees on how [`.rules` files](https://github.com/shrey16/kaabil-codex/blob/main/docs/execpolicy.md) are enforced.
...
```bash
pnpm --filter @kaabil/codex-shell-tool-mcp run build
node shell-tool-mcp/bin/mcp-server.js --version
```
...
codex --disable shell_tool \
  --config 'mcp_servers.bash={command = "node", args = ["/path/to/kaabil-codex/shell-tool-mcp/bin/mcp-server.js"]}'
```

To configure this permanently so you can use the MCP while running `codex` without additional command-line flags, add the following to your `~/.kaabil-codex/config.toml`:
...
Note when the MCP launcher runs, it selects the appropriate native binary to run based on the host OS/architecture. For the Bash wrapper, it inspects `/etc/os-release` on Linux or the Darwin major version on macOS to try to find the best match it has available. See [`bashSelection.ts`](https://github.com/shrey16/kaabil-codex/blob/main/shell-tool-mcp/src/bashSelection.ts) for details.

## MCP Client Requirements

This MCP server is designed to be used with [Kaabil Codex CLI](https://github.com/shrey16/kaabil-codex/blob/main/docs/getting-started.md), as it declares the following `capability` that Codex supports when acting as an MCP client:
...
This package wraps the `codex-exec-mcp-server` binary and its helpers so that the shell MCP can be invoked locally via the bundled launcher. It bundles:
...
See [the README in the Codex repo](https://github.com/shrey16/kaabil-codex/blob/main/codex-rs/exec-server/README.md) for details.
