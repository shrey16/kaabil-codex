...
//!
//! The history is stored at `~/.kaabil-codex/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside `~/.kaabil-codex`.
...
pub(crate) async fn append_entry(
    ...
    // Resolve `~/.kaabil-codex/history.jsonl` and ensure the parent directory exists.
